...
Put them inside `@layer i2p-base { }`. This matters: a layered declaration loses to an unlayered one whatever its specificity or its order, so Tailwind on a real page still wins every one of them, and the layer only shows through where there is no Tailwind. Without the layer, a rule such as `.i2p h2` would outrank `.text-4xl` and the block would start overriding the page.
...
Start with the five reset lines. Tailwind normally supplies these, and without them the list markers show through, the type falls back to a serif, and the content runs wider than the screen. That was measured: a first attempt without them put a block 36px past the right edge of a 1280px viewport.
    ...
    @layer i2p-base {
      .i2p,.i2p *,.i2p *::before,.i2p *::after{box-sizing:border-box}
      .i2p{font-family:ui-sans-serif,system-ui,-apple-system,"Segoe UI",Roboto,sans-serif;line-height:1.5}
      .i2p h1,.i2p h2,.i2p h3,.i2p h4,.i2p p,.i2p figure,.i2p blockquote{margin:0}
      .i2p ul,.i2p ol{margin:0;padding:0;list-style:none}
      .i2p img,.i2p svg,.i2p video{display:block;max-width:100%}
...
- Shapes a reader would notice at a glance: border radius, border color.
      ...
      .i2p{background-color:rgb(var(--i2p-surface));color:rgb(var(--i2p-ink));padding:5rem 1.25rem}
      .i2p .i2p-wrap{max-width:64rem;margin-inline:auto}
      .i2p .i2p-row{display:grid;gap:2.5rem}
      .i2p h2{font-size:2.25rem;line-height:1.15;font-weight:600}
      .i2p h3{font-size:1.25rem;font-weight:600}
...
| Variable | Used for |
| --- | --- |
| `--i2p-surface` | card and panel faces |
| `--i2p-band` | the quiet band behind the content |
| `--i2p-ink` | headings and primary copy |
| `--i2p-soft` | supporting copy |
| `--i2p-accent` | the one emphasis colour |
...
A tint such as `rgb(var(--i2p-accent) / .10)` is mostly the colour behind it, so judge contrast

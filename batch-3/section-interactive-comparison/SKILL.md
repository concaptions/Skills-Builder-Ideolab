...
description: A section comparing options against the same criteria, with one recommended. Use for a feature matrix, brand versus alternative, package comparison, or choice cards. Not for a comparison whose columns are switched by a script.
---
...
Put them inside `@layer sic-base { }`. This matters: a layered declaration loses to an unlayered one whatever its specificity or its order, so Tailwind on a real page still wins every one of them, and the layer only shows through where there is no Tailwind. Without the layer, a rule such as `.sic h2` would outrank `.text-4xl` and the block would start overriding the page.
...
Start with the five reset lines. Tailwind normally supplies these, and without them the list markers show through, the type falls back to a serif, and the content runs wider than the screen. That was measured: a first attempt without them put a block 36px past the right edge of a 1280px viewport.
    ...
    @layer sic-base {
      .sic,.sic *,.sic *::before,.sic *::after{box-sizing:border-box}
      .sic{font-family:ui-sans-serif,system-ui,-apple-system,"Segoe UI",Roboto,sans-serif;line-height:1.5}
      .sic h1,.sic h2,.sic h3,.sic h4,.sic p,.sic figure,.sic blockquote{margin:0}
      .sic ul,.sic ol{margin:0;padding:0;list-style:none}
      .sic img,.sic svg,.sic video{display:block;max-width:100%}
...
Then the block's own shape: the section's background, text color and padding, the wrapper's maximum width, every row or grid the layout depends on, and the heading and body type sizes.
      ...
      .sic{background-color:rgb(var(--sic-surface));color:rgb(var(--sic-ink));padding:3rem 1.25rem}
      .sic .sic-wrap{max-width:64rem;margin-inline:auto}
      .sic .sic-row{display:grid;gap:1.5rem}
      .sic h2{font-size:1.875rem;line-height:1.2;font-weight:600}
      .sic h3{font-size:1.125rem;font-weight:600}
      .sic p{font-size:1rem;line-height:1.625}
    }
...
Settings - Set options, criteria, recommended choice, toggle behavior, tooltips, and mobile stacking.

A straight answer to what is the difference, laid out so the reader can check every row themselves.

## When to use it

When a reader is genuinely choosing between two or three options and the differences matter.

## When not to use it

When the comparison is rigged. A table where the alternative fails every row persuades nobody and reads as an advert.

## Where it belongs on the page

After the options have been introduced and before the call to action.

## What may be changed

The options, the criteria, which one is recommended, and whether the layout is a table or a set of cards.

## What must not be changed

It is a real `<table>` with `<th scope>` on the headers, or a screen reader reads a run of unconnected cells. A tick is never the only thing in a cell: the yes or no is in text as well, because a shape has no meaning read aloud. The recommendation is stated in words.
...
- **Animation.** Column highlight, animated checks, toggle transition, hover explanation, or recommended-option emphasis.

## Motion

Rows fade in as the table arrives. The recommended column does not glow continuously, because a column that pulses while the reader is comparing is a distraction.

## Responsive and Accessibility

Below 768px the table becomes one card per option with the criteria repeated inside each, since a three column table at phone width is unreadable. That is a real layout, not a horizontal scroll.

## Defaults

Three options, five criteria, middle recommended, table above 768px, cards below.

## What breaks this block

A grid of divs instead of a table, which loses every row and column relationship. Ticks with no text. A comparison where the alternative fails everything.

## What a static block cannot do

A toggle that switches criteria, hover tooltips that explain a row, and a highlighted column that follows the pointer all need a script. A toggle can be built with a radio group and `:checked ~` if one is really needed.
...
| Variable | Used for |
| --- | --- |
| `--sic-surface` | card and panel faces |
| `--sic-band` | the quiet band behind the content |
| `--sic-ink` | headings and primary copy |
| `--sic-soft` | supporting copy |
| `--sic-accent` | the one emphasis color |
| `--sic-deep` | the dark ground |

The values are channel triplets, `15 23 42` rather than `#0f172a`, so an opacity suffix still works: `rgb(var(--sic-ink) / .70)`.
...
A tint such as `rgb(var(--sic-accent) / .10)` is mostly the color behind it, so judge contrast against the composited result.
